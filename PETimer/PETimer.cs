...
public class PETimer {
    ...
    public bool ReplaceTimerTask(int tid, Action<int> callback, double delay, PETimeUnit timeUnit = PETimeUnit.Millisecond, int count = 1) {
        if (timeUnit != PETimeUnit.Millisecond) {
            switch (timeUnit) {
                case PETimeUnit.Second:
                    delay = delay * 1000;
    ...
    public bool ReplaceFramerTask(int tid, Action<int> callback, int frame, int count = 1) {
